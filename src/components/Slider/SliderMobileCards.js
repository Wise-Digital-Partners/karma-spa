...
const StyledSlider = styled.div`
  ...
  .slick-prev,
  .slick-next {
    ${tw`top-auto bottom-0 transform translate-y-0 bg-white w-10 h-10 border border-solid border-gray-200 rounded-full transition-all duration-300 ease-linear z-10`}
    &:before {
      ${tw`hidden`}
    }
    i {
  ...
  .slick-dots {
    ...
    li {
      ...
      button {
          ${tw`bg-primary_400 rounded-full opacity-25 w-2 h-2 p-0 transition-all duration-300 ease-linear`}
          &:before {
            ${tw`hidden`}
          }
      } 
